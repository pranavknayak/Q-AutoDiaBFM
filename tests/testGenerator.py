...
def runTests(bugTestFiles, bugDatasetDirectoryName,
    ...
    for bugTestFile in bugTestFiles:
        ...
        bugTestFileProber = importlib.import_module(bugDatasetDirectoryName + "." + bugTestFileName[:-3],
...
def testIncorrectGate():
    ...
    bugTestFilesHandler = Path(bugDatasetDirectoryPath).glob('**/' + bugTypeName + '*.py')
    ...
    allTestsPassed = runTests(bugTestFiles, bugDatasetDirectoryName, 
                              bugDatasetDirectoryPath, characteristicBugMessage)
    if allTestsPassed:
...
def runAllTests(): 
    status = testIncorrectGate() | testIncorrectInit() | testIncorrectMeasurement() | testIncorrectQubitCount() | testIncorrectQubitOrder()
    if status == True:
